...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['unfollow'])) {
    if (!$isLoggedIn) {
        echo "<div class='error'>Connectez-vous pour ne plus suivre</div>";
    } else {
        $unfollowed_id = $profile_id;

        if ($unfollowed_id <= 0) {
            echo "<div class='error'>ID utilisateur invalide</div>";
        } elseif ($current_user_id == $unfollowed_id) {
            echo "<div class='error'>Vous ne pouvez pas vous désabonner de vous-même</div>";
        } else {
            // Vérifier si l'utilisateur est suivi
            $check_query = "SELECT * FROM follow WHERE followerUser_id = ? AND followedUser_id = ?";
            $check_stmt = mysqli_prepare($connexion, $check_query);
            mysqli_stmt_bind_param($check_stmt, "ii", $current_user_id, $unfollowed_id);
            mysqli_stmt_execute($check_stmt);
            $check_result = mysqli_stmt_get_result($check_stmt);
            ...
            if (mysqli_num_rows($check_result) === 0) {
                ...
                try {
                    ...
                    mysqli_stmt_bind_param($delete_stmt, "ii", $current_user_id, $unfollowed_id);
                    mysqli_stmt_execute($delete_stmt);

                    // 2. Réduire le nombre de followers pour l'utilisateur désabonné
                    $update_followers = "UPDATE users SET followers = followers - 1 WHERE user_id = ?";
                    $update_followers_stmt = mysqli_prepare($connexion, $update_followers);
                    mysqli_stmt_bind_param($update_followers_stmt, "i", $unfollowed_id);
                    mysqli_stmt_execute($update_followers_stmt);

                    // 3. Réduire le nombre de following pour l'utilisateur qui se désabonne
                    $update_following = "UPDATE users SET following = following - 1 WHERE user_id = ?";
                    $update_following_stmt = mysqli_prepare($connexion, $update_following);
                    mysqli_stmt_bind_param($update_following_stmt, "i", $current_user_id);
                    mysqli_stmt_execute($update_following_stmt);
...
<form method="POST" action="">
    <input type="hidden" name="unfollowed_id" value="<?= htmlspecialchars($profile_id) ?>">
    <input type="submit" name="unfollow" value="Ne plus suivre" <?= !$isLoggedIn ? '' : '' ?>>
</form>